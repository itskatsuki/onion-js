...
module.exports.run = async (bot, message, args, ops) => {
    ...
    if(!voiceChannel) return(`You aren't in a voice channel, please join one then try again.`)

    const reason = args.join(" ")
    if (reason.length < 1) return message.channel.send('Please provide a song after the command.')
